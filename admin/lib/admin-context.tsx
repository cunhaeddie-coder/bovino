...
export function roleColor(papel: AdminPapel): string { return ROLE_COLORS[papel] ?? "bg-slate-100 text-slate-600"; }
...
type AdminContextValue = { admin: AdminUser | null; loading: boolean; refresh: () => void };
...
export function AdminProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { load(); }, []);
  return <AdminContext.Provider value={{ admin, loading, refresh: load }}>{children}</AdminContext.Provider>;
}
